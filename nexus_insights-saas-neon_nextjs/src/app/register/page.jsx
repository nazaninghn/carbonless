...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (loginRes.ok) {
        try { const d = await loginRes.json(); if (d.access) { const { setAccessToken } = await import('@/lib/utils/api'); setAccessToken(d.access); } } catch {}
        ...
        <div className="grid gap-4 lg:grid-cols-[290px_minmax(0,1fr)] xl:grid-cols-[310px_minmax(0,1fr)]">
          ...
          <aside className="lg:sticky lg:top-4 lg:h-fit">
            <div className="rounded-[2rem] border border-[#302817]/10 bg-white/55 p-4 shadow-xl shadow-[#302817]/8 backdrop-blur-2xl">
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8F92A1]">Carbonless onboarding</p>
              ...
              <div className="mb-5">
                ...
                <div className="h-2 overflow-hidden rounded-full bg-[#302817]/10"><div className="h-full rounded-full bg-gradient-to-r from-[#8F92A1] to-[#7F8790] transition-all duration-500" style={{ width: `${progress}%` }} /></div>
              </div>
              <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-1">
                {steps.map(step => { const Icon = step.icon; const active = currentSection === step.id; const done = currentSection > step.id; return (
                  <button key={step.id} type="button" onClick={() => goToSection(step.id)} className={`group flex min-w-0 items-center gap-3 rounded-2xl border p-3 text-left transition ${active ? 'border-[#8F92A1]/45 bg-[#8F92A1]/14 shadow-lg shadow-[#302817]/5' : done ? 'border-[#7F8790]/35 bg-white/65 hover:border-[#8F92A1]/45' : 'border-[#302817]/10 bg-white/35 hover:bg-white/60'}`}>
                    <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl transition ${active ? 'bg-[#8F92A1] text-white' : done ? 'bg-[#7F8790] text-white' : 'bg-[#F8F8F8] text-[#302817]/55 ring-1 ring-[#302817]/10'}`}>{done ? <Check className="h-5 w-5" /> : <Icon className="h-5 w-5" />}</span>
              ...
              <div className="mt-5 rounded-3xl border border-[#302817]/10 bg-[#302817] p-4 text-[#F9EFE5] shadow-xl shadow-[#302817]/15">
                <div className="flex items-center gap-2 text-sm font-bold"><ShieldCheck className="h-4 w-4 text-[#8F92A1]" />{language === 'tr' ? 'Güvenli kayıt' : 'Secure registration'}</div>
          ...
          <form onSubmit={handleSubmit} className="min-w-0 rounded-[2rem] border border-[#302817]/10 bg-white/55 p-4 shadow-xl shadow-[#302817]/8 backdrop-blur-2xl sm:p-5 lg:p-6">
            <div className="mb-5 flex flex-col justify-between gap-3 border-b border-[#302817]/10 pb-4 sm:flex-row sm:items-start">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8F92A1]">{language === 'tr' ? `Adım ${currentSection} / 3` : `Step ${currentSection} / 3`}</p>
              ...
              <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[#302817]/10 bg-white/55 px-4 py-2 text-xs font-bold text-[#302817] shadow-sm backdrop-blur-xl"><BadgeCheck className="h-4 w-4 text-[#8F92A1]" />{language === 'tr' ? 'Zorunlu *' : 'Required *'}</div>
            </div>
            ...
            {currentSection === 1 && (
              <div className="space-y-4">
                ...
                <Panel title={language === 'tr' ? 'Sektör' : 'Sector'} icon={Globe2}>
                  <div><Label required>{language === 'tr' ? 'NACE Kodu' : 'NACE Code'}</Label><div className="relative"><input type="text" value={naceSearch} onChange={e => { setNaceSearch(e.target.value); setNaceOpen(true); }} onFocus={() => setNaceOpen(true)} placeholder={language === 'tr' ? 'Kod veya sektör adı...' : 'Code or sector name...'} className="field-premium" />{formData.naceCode && <div className="mt-2 flex items-center justify-between rounded-2xl border border-[#8F92A1]/30 bg-[#8F92A1]/12 px-4 py-3 text-sm"><span><b>{formData.naceCode}</b> — {ALL_NACE_CODES.find(n => n.code === formData.naceCode)?.[language === 'tr' ? 'tr' : 'en'] || ''}</span><button type="button" onClick={() => { handleInputChange('naceCode', ''); setNaceSearch(''); }} className="text-xs text-red-500">✕</button></div>}{naceOpen && naceSearch.length > 0 && <div className="absolute left-0 right-0 top-full z-50 mt-2 max-h-56 overflow-y-auto rounded-2xl border border-[#302817]/10 bg-white p-2 shadow-2xl">{ALL_NACE_CODES.filter(n => { const q = naceSearch.toLowerCase(); return n.code.toLowerCase().includes(q) || n.tr.toLowerCase().includes(q) || n.en.toLowerCase().includes(q); }).slice(0, 12).map(n => <button key={n.code} type="button" onClick={() => { handleInputChange('naceCode', n.code); setNaceSearch(''); setNaceOpen(false); }} className="w-full rounded-xl px-4 py-3 text-left text-sm hover:bg-[#8F92A1]/10"><b className="text-[#7F8790]">{n.code}</b> <span className="text-[#302817]/70">{language === 'tr' ? n.tr : n.en}</span></button>)}</div>}</div></div>
...
/* Sub-components */
function Panel({ title, icon: Icon, children }) { return (<section className="rounded-[1.5rem] border border-[#302817]/10 bg-white/50 p-4 shadow-sm backdrop-blur-xl sm:p-5"><div className="mb-4 flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#8F92A1]/14 text-[#7F8790] ring-1 ring-[#8F92A1]/25"><Icon className="h-5 w-5" /></div><h3 className="text-base font-bold text-[#302817] sm:text-lg">{title}</h3></div>{children}</section>); }
function Label({ children, required }) { return <label className="mb-2 block text-sm font-bold text-[#302817]/75">{children} {required && <span className="text-[#8F92A1]">*</span>}</label>; }
function TextField({ label, required, icon: Icon, ...props }) { return (<div><Label required={required}>{label}</Label><div className="group flex items-center gap-3 rounded-2xl border border-[#302817]/10 bg-white/55 px-4 py-3 shadow-sm backdrop-blur-xl transition focus-within:border-[#8F92A1]/60 focus-within:ring-4 focus-within:ring-[#8F92A1]/20">{Icon && <Icon className="h-5 w-5 shrink-0 text-[#7F8790]" />}<input {...props} className="w-full bg-transparent text-sm font-semibold text-[#302817] outline-none placeholder:text-[#302817]/35" /></div></div>); }
...
function RadioOption({ name, option, value, onChange, label }) { const active = value === option; return (<label className={`flex cursor-pointer items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-bold transition ${active ? 'border-[#8F92A1]/55 bg-[#8F92A1]/14 text-[#302817] ring-4 ring-[#8F92A1]/15' : 'border-[#302817]/10 bg-white/55 text-[#302817]/70 hover:border-[#8F92A1]/45'}`}><input type="radio" name={name} value={option} checked={active} onChange={e => onChange(e.target.value)} className="h-4 w-4 accent-[#8F92A1]" />{label}</label>); }
function CheckCard({ checked, onChange, label }) { return (<label className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 text-sm font-bold transition ${checked ? 'border-[#8F92A1]/55 bg-[#8F92A1]/14 text-[#302817] ring-4 ring-[#8F92A1]/15' : 'border-[#302817]/10 bg-white/55 text-[#302817]/70 hover:border-[#8F92A1]/45'}`}><input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} className="mt-0.5 h-5 w-5 accent-[#8F92A1]" /><span>{label}</span></label>); }
