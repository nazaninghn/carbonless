...
// current year" — meaning from January 2027 onward, a user would have had no
// way to select the actual current year at all, and the "data may be
// incomplete" warning would have stayed permanently stuck on 2026. Computed
// once at module load so the question always offers the real current year.
...
export const CARBONIQ_QUESTIONS = [
  ...
  },
  {
    ...
    options: Array.from({ length: 7 }, (_, i) => CURRENT_YEAR - 6 + i).map((year) => ({
      value: String(year),
      label: {
        tr: year === CURRENT_YEAR ? `${year} (cari yıl — veri eksik olabilir)` : String(year),
        en: year === CURRENT_YEAR ? `${year} (current year — data may be incomplete)` : String(year),
      },
    })),
    validate: {
      requiredMessage: {
    ...
    assumptions: [
      {
        when: { equals: String(CURRENT_YEAR) },
        type: 'A',
    ...
    warning: {
      when: { equals: String(CURRENT_YEAR) },
      text: {
        tr: `${CURRENT_YEAR} henüz tamamlanmadı. Yıl sonu verileriniz eksik olabilir — bazı kalemlerde tahmini veri kullanmak gerekebilir. Bu durum raporunuzda belgelenecek.`,
        en: `${CURRENT_YEAR} is not complete yet. Year-end data may be incomplete — some items may require estimated data. This will be documented in your report.`,
      },
    },
    next: 'A5',
  },
  ...
  },
  {
    ...
    options: Array.from({ length: 11 }, (_, i) => CURRENT_YEAR - 11 + i).map((year) => ({
      value: String(year),
  ...
  },
  {
    ...
    subtype: 'numeric',
    required: true,
    reportField: 'org_boundary.facility_count',
    prefillFrom: 'B4',
    text: {
      tr: 'Kaç fiziksel tesisiniz var? (B4\'ten otomatik gelir)',
      en: 'How many physical facilities do you have? (auto-filled from B4)',
    },
    placeholder: {
      tr: 'Örn: 3',
      en: 'e.g. 3',
    ...
    helper: {
      tr: 'Şirketinizin sahibi olduğu veya operasyonel kontrolünü elinde bulundurduğu tüm fiziksel konumları (fabrika, ofis, depo, vb.) sayınız. Bir önceki soruda verilen değer otomatik doldurulur.',
      en: 'Count all physical locations (factory, office, warehouse, etc.) owned or under operational control of your company. The value entered earlier is auto-filled.',
    },
    validate: {
      requiredMessage: { tr: 'Lütfen tesis sayısını girin.', en: 'Please enter the number of facilities.' },
      minValue: 1,
      minValueMessage: { tr: 'En az 1 tesis girilmelidir.', en: 'At least 1 facility must be entered.' },
    },
    systemMessages: {
      filled: {
        tr: 'Tesis sayısı alındı. Şimdi her tesis için ad ve ülke bilgisini gireceğiz.',
        en: 'Facility count received. We will now enter the name and country for each facility.',
      },
    },
  ...
  },
  {
    ...
    loopSource: '2A-1',
    loopNext: '2A-4',
    reportField: 'org_boundary.facilities[N].activity_type',
    text: {
      tr: '[Tesis N] Bu tesis ne tür faaliyet yürütüyor?',
      en: '[Facility N] What type of activity does this facility carry out?',
    },
    placeholder: {
      tr: 'Örn: Üretim / montaj, Genel merkez ofisi, Lojistik deposu',
      en: 'e.g. Manufacturing / assembly, Head office, Logistics warehouse',
    },
    helper: {
      tr: 'Tesisin ana faaliyetini kısaca tanımlayın (üretim, ofis, depo, Ar-Ge vb.). Bu bilgi kapsam sınırı açıklamasında kullanılır.',
      en: 'Briefly describe the main activity of the facility (manufacturing, office, warehouse, R&D, etc.). Used in the scope boundary description.',
    },
    validate: {
      requiredMessage: { tr: 'Faaliyet türü zorunludur.', en: 'Activity type is required.' },
  ...
  },
  {
    ...
    validate: {
      minValue: 0,
      maxValue: 100,
      rangeMessage: { tr: 'Hisse oranı 0–100 arasında olmalıdır.', en: 'Equity share must be between 0 and 100.' },
    },
    units: ['%'],
    next: '2B-OC1',
  },
  ...
  },
  {
    ...
    helper: {
      ...
      en: 'Day-to-day operations management is important for correctly establishing the inventory boundary. Select the most applicable option.',
    },
    options: [
  ...
  },
  {
    ...
    helper: {
      ...
      en: 'Financial control covers only facilities where you have the authority to manage both financial AND operating policies.',
    },
    options: [
      { value: 'yes', label: { tr: 'Evet — her ikisi de bende', en: 'Yes — I have both' } },
      { value: 'no', label: { tr: 'Hayır — tam yetkim yok', en: 'No — I do not have full authority' } },
    ],
    validate: {
      requiredMessage: { tr: 'Lütfen bir seçenek belirtin.', en: 'Please select an option.' },
    },
  ...
  },
  {
    ...
    conditionalShow: { questionId: 'D3', includesValue: 'equity_share' },
    prefillFrom: '2A-5',
    reportField: 'org_boundary.facilities[N].equity_pct',
    text: {
      tr: '[Her varlık] Bu varlıktaki hisse oranınız (2A-5\'ten otomatik)',
      en: '[Each asset] Your equity share in this asset (auto-filled from 2A-5)',
    },
    placeholder: {
      tr: 'Örn: 75',
      en: 'e.g. 75',
    ...
    helper: {
      tr: 'Hisse Payı yaklaşımı seçildiğinde bu varlığa ait emisyonlar girilen oran kadar envantere yansıtılır. 2A-5\'te girilen değer otomatik doldurulur.',
      en: 'When the Equity Share approach is selected, emissions from this asset are reflected in the inventory proportional to the entered rate. Auto-filled from 2A-5.',
    },
    validate: {
      minValue: 0,
      maxValue: 100,
  ...
  },
  {
    ...
    helper: {
      tr: 'ISO 14064-1, kapsam dışı bırakılan her varlık için gerekçe belgelenmesini zorunlu kılar. En uygun kategoriyi seçin.',
      en: 'ISO 14064-1 requires documented justification for every excluded asset. Select the most applicable category.',
    },
    options: [
      { value: 'no_control', label: { tr: 'Kontrol/sahiplik bulunmuyor', en: 'No control / ownership' } },
      { value: 'no_data', label: { tr: 'Emisyon verisi mevcut değil', en: 'Emission data not available' } },
      { value: 'immaterial', label: { tr: 'Materyal değil (düşük emisyon büyüklüğü)', en: 'Not material (low emission magnitude)' } },
      { value: 'operational_boundary', label: { tr: 'Operasyonel sınır dışında', en: 'Outside operational boundary' } },
      { value: 'sold_divested', label: { tr: 'Satıldı / elden çıkarıldı', en: 'Sold / divested' } },
      { value: 'other', label: { tr: 'Diğer (açıklama ekle)', en: 'Other (add explanation)' } },
    ],
    validate: {
      requiredMessage: { tr: 'Lütfen dışlama gerekçe kategorisini seçin.', en: 'Please select the exclusion reason category.' },
    },
    next: '2C-2',
  },
  {
    ...
    helper: {
      tr: 'Kesin değer bilinmese de yaklaşık büyüklük, materyal olmayan dışlamaların belgelenmesi için gereklidir (ISO 14064-1 §5.1).',
      en: 'Even if the exact value is unknown, an approximate magnitude is required to document immaterial exclusions (ISO 14064-1 §5.1).',
    },
    options: [
      { value: 'very_low', label: { tr: 'Çok düşük (< 1 tCO2e / yıl)', en: 'Very low (< 1 tCO2e / year)' } },
      { value: 'low', label: { tr: 'Düşük (1–10 tCO2e / yıl)', en: 'Low (1–10 tCO2e / year)' } },
      { value: 'medium', label: { tr: 'Orta (10–100 tCO2e / yıl)', en: 'Medium (10–100 tCO2e / year)' } },
      { value: 'high', label: { tr: 'Yüksek (> 100 tCO2e / yıl)', en: 'High (> 100 tCO2e / year)' } },
    ],
    systemMessages: {
      high: {
        tr: 'Uyarı: Yüksek emisyonlu bir varlığın kapsam dışı bırakılması raporun materyalitesini olumsuz etkileyebilir. ISO 14064-1 kapsamında gerekçenizi dikkatlice belgeleyin.',
        en: 'Warning: Excluding a high-emission asset may negatively affect the materiality of the report. Document your justification carefully under ISO 14064-1.',
      },
    },
    next: '2C-3',
  },
  {
    ...
    helper: {
      tr: 'İlerideki raporlama planınızı belirtmek, paydaşlara şeffaflık sağlar ve sürekli iyileştirme taahhüdünü gösterir.',
      en: 'Indicating your future reporting plan provides transparency to stakeholders and demonstrates a commitment to continuous improvement.',
    },
    options: [
      { value: 'yes_next_year', label: { tr: 'Evet — gelecek rapor döneminde dahil edilecek', en: 'Yes — will be included in the next reporting period' } },
      { value: 'yes_future', label: { tr: 'Evet — ileride dahil etmeyi planlıyorum (belirli yıl yok)', en: 'Yes — planning to include in the future (no specific year)' } },
      { value: 'no', label: { tr: 'Hayır — dahil etme planım yok', en: 'No — no plans to include' } },
    ],
    systemMessages: {
      yes_next_year: {
        tr: 'Gelecek yıl dahil etme planı kaydedildi. Bir sonraki raporlama döneminde bu varlık envantere eklenecek.',
        en: 'Next-year inclusion plan recorded. This asset will be added to the inventory in the next reporting period.',
      },
      complete: {
  ...
  },
  {
    ...
    text: {
      tr: 'Yakıtlarınız için tedarikçinin emisyon faktörü belgesi var mı?',
      en: 'Do you have a supplier emission factor document for your fuels?',
    },
    helper: {
      tr: 'Tedarikçinizin ısıl değer belgesi, yakıt analiz raporu veya çevre beyanı varsa bu Seviye 1 (en doğru) veri sayılır. Sonraki adımda belge detaylarını gireceksiniz.',
      en: 'A calorific value document, fuel analysis report or environmental declaration from your supplier counts as Level 1 (most accurate) data. You will enter the document details in the next step.',
    },
    options: [
    ...
    systemMessages: {
      no: {
        tr: 'Önerilen emisyon faktörü: DEFRA 2023 doğalgaz EF — 0.2023 kg CO₂e/kWh (TTW). Kaynak: DEFRA 2023. Bu değer hesaplamada kullanılacak.',
        en: 'Suggested emission factor: DEFRA 2023 natural gas EF — 0.2023 kg CO₂e/kWh (TTW). Source: DEFRA 2023. This value will be used in the calculation.',
      },
    },
    nextByValue: {
      yes: '3A-EF-a',
  ...
  },
  {
    ...
    type: 'compound',
    required: true,
    reportField: 'scope1.stationary_combustion.ef_document',
    conditionalShow: { questionId: '3A-EF', equals: 'yes' },
    text: {
      tr: 'Tedarikçi emisyon faktörü belgesini girin',
      en: 'Enter supplier emission factor document details',
    },
    helper: {
      tr: 'Tedarikçi belgesindeki emisyon faktörü değerini, birimini, kaynak adını ve yılını girin. Bu bilgiler ISO 14064-1 uyumu için raporunuzda belgelenecek.',
      en: 'Enter the emission factor value, unit, source name and year from the supplier document. This information will be documented in your report for ISO 14064-1 compliance.',
    },
    fields: [
      {
        id: 'ef_value',
      ...
      },
      {
        ...
        options: [
          ...
          { value: 'kgCO2e_litre', label: { tr: 'kg CO₂e / litre', en: 'kg CO₂e / litre' } },
          { value: 'kgCO2e_kg', label: { tr: 'kg CO₂e / kg', en: 'kg CO₂e / kg' } },
          { value: 'kgCO2e_m3', label: { tr: 'kg CO₂e / m³', en: 'kg CO₂e / m³' } },
          { value: 'other', label: { tr: 'Diğer', en: 'Other' } },
        ],
      },
      {
        ...
        placeholder: { tr: 'Örn: XYZ Doğalgaz A.Ş. — Emisyon Beyanı 2023', en: 'e.g. XYZ Natural Gas Ltd. — Emission Declaration 2023' },
        maxLength: 200,
      },
      {
        id: 'ef_year',
    ...
    validate: {
      requiredMessage: { tr: 'Lütfen tüm EF belge alanlarını doldurun.', en: 'Please fill in all EF document fields.' },
    },
    next: '3B-0',
  },
  ...
  },
  {
    ...
    text: {
      tr: 'Araçlarınız için tedarikçi veya üretici emisyon faktörü belgesi var mı?',
      en: 'Do you have a supplier or manufacturer emission factor document for your vehicles?',
    },
    helper: {
      tr: 'Araç üreticisinin LCA belgesi veya yakıt tedarikçisinin emisyon beyanı varsa bu Seviye 1 veri sayılır. Sonraki adımda belge detaylarını gireceksiniz.',
      en: 'A vehicle manufacturer LCA document or fuel supplier emission declaration counts as Level 1 data. You will enter the document details in the next step.',
    },
    options: [
    ...
    systemMessages: {
      no: {
        tr: 'Araç tipi ve yakıt türüne göre DEFRA 2023 emisyon faktörü tablosundan uygun değer kullanılacak.',
        en: 'An appropriate value from the DEFRA 2023 emission factor table will be used based on vehicle type and fuel type.',
      },
    },
    nextByValue: {
      yes: '3B-EF-a',
  ...
  },
  {
    ...
    type: 'compound',
    required: true,
    reportField: 'scope1.mobile_combustion.ef_document',
    conditionalShow: { questionId: '3B-EF', equals: 'yes' },
    text: {
      tr: 'Araç emisyon faktörü belgesini girin',
      en: 'Enter vehicle emission factor document details',
    },
    helper: {
      tr: 'Üretici LCA belgesi veya yakıt tedarikçisi emisyon beyanındaki EF değerini, birimini, kaynağını ve yılını girin.',
      en: 'Enter the EF value, unit, source and year from the manufacturer LCA document or fuel supplier emission declaration.',
    },
    fields: [
      {
        id: 'ef_value',
        ...
        required: true,
        label: { tr: 'Emisyon faktörü değeri', en: 'Emission factor value' },
        placeholder: { tr: 'Örn: 2.6824', en: 'e.g. 2.6824' },
      },
      {
        id: 'ef_unit',
        type: 'select',
        ...
        options: [
          { value: 'kgCO2e_litre', label: { tr: 'kg CO₂e / litre', en: 'kg CO₂e / litre' } },
          { value: 'kgCO2e_km', label: { tr: 'kg CO₂e / km', en: 'kg CO₂e / km' } },
          { value: 'kgCO2e_kWh', label: { tr: 'kg CO₂e / kWh', en: 'kg CO₂e / kWh' } },
          { value: 'kgCO2e_kg', label: { tr: 'kg CO₂e / kg', en: 'kg CO₂e / kg' } },
          { value: 'other', label: { tr: 'Diğer', en: 'Other' } },
        ],
      },
      {
        ...
        placeholder: { tr: 'Örn: Ford Motor Company — LCA Raporu 2023', en: 'e.g. Ford Motor Company — LCA Report 2023' },
        maxLength: 200,
      },
      {
        id: 'ef_year',
  ...
  },
  {
    ...
    text: {
      tr: 'Prosesleriniz için tesis spesifik emisyon faktörü var mı?',
      en: 'Do you have a facility-specific emission factor for your processes?',
    },
    helper: {
      tr: 'Tesis ölçümüne dayalı veya mühendislik hesabından üretilen bir EF en doğru sonucu verir. Yoksa sistem sektör protokolüne (WBCSD/World Steel/IPCC) göre öneride bulunur.',
      en: 'An EF based on facility measurement or engineering calculation gives the most accurate result. If not available, the system will suggest based on sector protocol (WBCSD/World Steel/IPCC).',
    },
    options: [
    ...
    systemMessages: {
      no: {
        tr: 'Sektörünüze uygun varsayılan emisyon faktörü kullanılacak: Çimento → WBCSD Protokolü, Çelik → World Steel, Diğer → IPCC Tier 1.',
        en: 'A default emission factor appropriate for your sector will be used: Cement → WBCSD Protocol, Steel → World Steel, Other → IPCC Tier 1.',
      },
    },
    nextByValue: {
      yes: '3C-EF-a',
  ...
  },
  {
    ...
    type: 'compound',
    required: true,
    reportField: 'scope1.process_emissions.ef_document',
    conditionalShow: { questionId: '3C-EF', equals: 'yes' },
    text: {
      tr: 'Tesis spesifik emisyon faktörünü girin',
      en: 'Enter facility-specific emission factor details',
    },
    helper: {
      tr: 'Tesis ölçümü veya mühendislik hesabına dayalı emisyon faktörü değerini, birimini, kaynağını ve yılını girin.',
      en: 'Enter the emission factor value, unit, source and year based on facility measurement or engineering calculation.',
    },
    fields: [
      {
        id: 'ef_value',
        ...
        required: true,
        label: { tr: 'Emisyon faktörü değeri', en: 'Emission factor value' },
        placeholder: { tr: 'Örn: 0.8250', en: 'e.g. 0.8250' },
      },
      {
        id: 'ef_unit',
        type: 'select',
        ...
        label: { tr: 'Birim', en: 'Unit' },
        options: [
          { value: 'kgCO2e_tonne_product', label: { tr: 'kg CO₂e / ton ürün', en: 'kg CO₂e / tonne product' } },
          { value: 'tCO2e_tonne_product', label: { tr: 't CO₂e / ton ürün', en: 't CO₂e / tonne product' } },
          { value: 'kgCO2e_kg', label: { tr: 'kg CO₂e / kg', en: 'kg CO₂e / kg' } },
          { value: 'kgCO2e_m3', label: { tr: 'kg CO₂e / m³', en: 'kg CO₂e / m³' } },
          { value: 'other', label: { tr: 'Diğer', en: 'Other' } },
        ],
      },
      {
        ...
        placeholder: { tr: 'Örn: Tesis çıkış bacası ölçümü — Çevre Mühendisliği Raporu 2023', en: 'e.g. Stack measurement at facility — Environmental Engineering Report 2023' },
        maxLength: 200,
      },
      {
        id: 'ef_year',
