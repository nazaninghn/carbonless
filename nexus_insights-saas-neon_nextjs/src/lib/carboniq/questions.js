...
export const CARBONIQ_QUESTIONS = [
  ...
  // ── BLOCK B ─ Activity Profile ─────────────────────────────────────────────
  {
    ...
    maxLength: 10,
    numericOnly: true,
    reportField: 'company.employee_count',
    text: {
      tr: 'Şirketinizde kaç kişi çalışmaktadır?',
      en: 'How many people work at your company?',
    },
    placeholder: { tr: 'Örn: 250', en: 'Example: 250' },
    helper: {
      tr: 'Raporlama yılı sonu itibarıyla toplam çalışan sayısını yazın. Tam zamanlı eşdeğer (FTE) hesabı kullanabilirsiniz.',
      en: 'Enter the total number of employees as of the end of the reporting year. You may use full-time equivalent (FTE) figures.',
    },
    validate: {
      requiredMessage: { tr: 'Çalışan sayısı zorunludur.', en: 'Employee count is required.' },
  ...
  },
  {
    ...
    text: {
      tr: 'Şirketinizin yıllık cirosunu belirtiniz.',
  ...
  },
  {
    ...
    options: [
      { value: 'manufacturing', label: { tr: 'Üretim / İmalat', en: 'Manufacturing' } },
      { value: 'construction', label: { tr: 'İnşaat', en: 'Construction' } },
      { value: 'retail', label: { tr: 'Perakende / Ticaret', en: 'Retail / Trade' } },
      { value: 'logistics', label: { tr: 'Lojistik / Nakliye', en: 'Logistics / Transportation' } },
      { value: 'it_tech', label: { tr: 'Bilgi Teknolojileri', en: 'Information Technology' } },
      { value: 'finance', label: { tr: 'Finans / Bankacılık', en: 'Finance / Banking' } },
      { value: 'healthcare', label: { tr: 'Sağlık', en: 'Healthcare' } },
      { value: 'energy', label: { tr: 'Enerji', en: 'Energy' } },
      { value: 'agriculture', label: { tr: 'Tarım', en: 'Agriculture' } },
      { value: 'hospitality', label: { tr: 'Turizm / Konaklama', en: 'Tourism / Hospitality' } },
      { value: 'education', label: { tr: 'Eğitim', en: 'Education' } },
      { value: 'public', label: { tr: 'Kamu / Hizmet', en: 'Public / Government' } },
  ...
  },
  {
    ...
    type: 'multi_select',
    required: true,
    reportField: 'company.countries',
    text: {
      tr: 'Faaliyetlerinizin yürütüldüğü ülkeleri seçin.',
      en: 'Select all countries where your company operates.',
    },
    helper: {
      tr: 'Birden fazla ülke seçebilirsiniz. Emisyon faktörü hesabı için ülke bazında farklı katsayılar kullanılacak.',
      en: 'You may select multiple countries. Country-specific emission factors will be applied.',
    },
    options: [
      { value: 'TR', label: { tr: 'Türkiye', en: 'Turkey' } },
      { value: 'DE', label: { tr: 'Almanya', en: 'Germany' } },
      { value: 'GB', label: { tr: 'Birleşik Krallık', en: 'United Kingdom' } },
      { value: 'US', label: { tr: 'ABD', en: 'United States' } },
      { value: 'NL', label: { tr: 'Hollanda', en: 'Netherlands' } },
      { value: 'FR', label: { tr: 'Fransa', en: 'France' } },
      { value: 'IT', label: { tr: 'İtalya', en: 'Italy' } },
      { value: 'AE', label: { tr: 'BAE', en: 'UAE' } },
      { value: 'OTHER', label: { tr: 'Diğer', en: 'Other' } },
    ],
    validate: {
      requiredMessage: { tr: 'En az bir ülke seçin.', en: 'Please select at least one country.' },
    },
    next: 'B6',
  },
  {
    ...
    helper: {
      tr: 'Bu açıklama raporun "Şirket Profili" bölümünde kullanılacak. En fazla 500 karakter.',
      en: 'This description will be used in the "Company Profile" section of the report. Maximum 500 characters.',
    },
    validate: {
      requiredMessage: { tr: 'Faaliyet açıklaması zorunludur.', en: 'Business activities description is required.' },
      maxLengthMessage: { tr: 'En fazla 500 karakter olabilir.', en: 'Maximum 500 characters allowed.' },
    },
    next: 'C1',
  },
  ...
  // ── BLOCK C ─ Structural Information ──────────────────────────────────────
  {
    ...
    type: 'single_select',
    required: true,
    reportField: 'company.org_structure',
    text: {
      tr: 'Şirketinizin organizasyonel yapısı nedir?',
      en: 'What is the organizational structure of your company?',
    },
    helper: {
      tr: 'Bu bilgi organizasyon sınırının belirlenmesinde kullanılır.',
      en: 'This information is used to define the organizational boundary.',
    },
    options: [
      { value: 'standalone', label: { tr: 'Bağımsız şirket (grup veya bağlı ortaklık yok)', en: 'Standalone company (no group or subsidiaries)' } },
      { value: 'parent', label: { tr: 'Ana/Holding şirket (bağlı ortaklıkları var)', en: 'Parent/Holding company (has subsidiaries)' } },
      { value: 'subsidiary', label: { tr: 'Bağlı ortaklık / iştiraki (bir gruba bağlı)', en: 'Subsidiary / affiliate (part of a group)' } },
      { value: 'jv', label: { tr: 'Ortak girişim (joint venture)', en: 'Joint venture' } },
    ],
    validate: {
      requiredMessage: { tr: 'Lütfen organizasyon yapısını seçin.', en: 'Please select the organizational structure.' },
    },
    next: 'C2',
  },
  {
    ...
    type: 'single_select',
    required: true,
    reportField: 'company.subsidiaries_included',
    text: {
      tr: 'Bağlı ortaklıklar / iştirakler bu GHG envanterine dahil edilecek mi?',
      en: 'Will subsidiaries / affiliates be included in this GHG inventory?',
    },
    helper: {
      tr: 'Dahil etmek istediğiniz bağlı ortaklıklar varsa evet seçin. Kapsam dışı kalanlar 6. Aşama\'da belgelenecek.',
      en: 'Select yes if you want to include any subsidiaries. Excluded entities will be documented in Stage 6.',
    },
    options: [
      { value: 'yes_all', label: { tr: 'Evet — tüm bağlı ortaklıklar dahil', en: 'Yes — all subsidiaries included' } },
      { value: 'yes_partial', label: { tr: 'Evet — bazı bağlı ortaklıklar dahil', en: 'Yes — some subsidiaries included' } },
      { value: 'no', label: { tr: 'Hayır — yalnızca ana şirket', en: 'No — parent company only' } },
    ],
    validate: {
  ...
  },
  {
    ...
    type: 'single_select',
    required: true,
    reportField: 'company.consolidation_approach',
    text: {
      tr: 'GHG envanter sınırı için hangi konsolidasyon yaklaşımını kullanacaksınız?',
      en: 'Which consolidation approach will you use for the GHG inventory boundary?',
    },
    helper: {
    ...
    options: [
      { value: 'operational_control', label: { tr: 'Operasyonel kontrol', en: 'Operational control' } },
      { value: 'financial_control', label: { tr: 'Finansal kontrol', en: 'Financial control' } },
      { value: 'equity_share', label: { tr: 'Hisse oranı (equity share)', en: 'Equity share' } },
    ],
    validate: {
      requiredMessage: { tr: 'Lütfen bir konsolidasyon yaklaşımı seçin.', en: 'Please select a consolidation approach.' },
    },
    systemMessages: {
      operational_control: {
  ...
  },
  {
    ...
    required: false,
    text: {
      tr: 'Veri toplama süreci hakkında bilgilendirme',
      en: 'Information about the data collection process',
    },
    helper: {
      tr: 'Bir sonraki adımda GHG niceleme yönteminizi belirleyeceğiz. ISO 14064-1 iki temel yöntem tanımlar: ölçüm (doğrudan) ve hesaplama (aktivite verisi × emisyon faktörü). Çoğu şirket hesaplama yöntemini kullanır.',
      en: 'In the next step, we will determine your GHG quantification method. ISO 14064-1 defines two main methods: measurement (direct) and calculation (activity data × emission factor). Most companies use the calculation method.',
    },
    next: 'D3',
  },
  {
    ...
    type: 'single_select',
    required: true,
    reportField: 'report.quantification_method',
    text: {
      tr: 'GHG emisyonlarını niceleme yönteminiz nedir?',
      en: 'What is your GHG emission quantification method?',
    },
    helper: {
      tr: 'Raporunuzda metodoloji bölümünde açıklanacak.',
      en: 'This will be explained in the methodology section of your report.',
    },
    options: [
      { value: 'calculation', label: { tr: 'Hesaplama (Aktivite verisi × Emisyon faktörü) — önerilen', en: 'Calculation (Activity data × Emission factor) — recommended' } },
      { value: 'measurement', label: { tr: 'Doğrudan ölçüm (CEMS veya analitik)', en: 'Direct measurement (CEMS or analytical)' } },
      { value: 'mixed', label: { tr: 'Karma (bazı kaynaklar ölçüm, diğerleri hesaplama)', en: 'Mixed (measurement for some sources, calculation for others)' } },
    ],
    validate: {
      requiredMessage: { tr: 'Lütfen bir niceleme yöntemi seçin.', en: 'Please select a quantification method.' },
    },
    next: 'D4',
  },
  {
    ...
    type: 'single_select',
    required: false,
    reportField: 'report.verification_plan',
    text: {
      tr: 'Bu raporu bağımsız doğrulamaya (verifikasyon) tabi tutmayı planlıyor musunuz?',
      en: 'Do you plan to have this report independently verified?',
    },
    helper: {
      tr: 'Doğrulama bazı yasal zorunluluklar veya çerçeveler (CDP, CSRD) için gerekli olabilir. Planlamıyorsanız atlayabilirsiniz.',
      en: 'Verification may be required for certain legal obligations or frameworks (CDP, CSRD). You may skip if not planned.',
    },
    options: [
      { value: 'yes_limited', label: { tr: 'Evet — sınırlı güvence (limited assurance)', en: 'Yes — limited assurance' } },
      { value: 'yes_reasonable', label: { tr: 'Evet — makul güvence (reasonable assurance)', en: 'Yes — reasonable assurance' } },
      { value: 'no', label: { tr: 'Hayır — iç kullanım için', en: 'No — for internal use only' } },
      { value: 'undecided', label: { tr: 'Henüz karar vermedim', en: 'Not decided yet' } },
    ],
    next: '2A-0',
  },
