...
export default function PasswordChange({ language }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 max-w-md">
      <div>
        <label className="block text-xs text-graphite mb-1">{language === 'tr' ? 'Mevcut Şifre' : 'Current Password'}</label>
        <input type="password" value={oldPw} onChange={e => setOldPw(e.target.value)} className="w-full px-3 py-2 border border-black/[0.06] rounded-lg text-sm" required />
      </div>
      <div>
        <label className="block text-xs text-graphite mb-1">{language === 'tr' ? 'Yeni Şifre' : 'New Password'}</label>
        <input type="password" value={newPw} onChange={e => setNewPw(e.target.value)} className="w-full px-3 py-2 border border-black/[0.06] rounded-lg text-sm" required />
        <PasswordStrengthIndicator password={newPw} language={language} />
      </div>
      <div>
        <label className="block text-xs text-graphite mb-1">{language === 'tr' ? 'Yeni Şifre (Tekrar)' : 'Confirm New Password'}</label>
        <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} className="w-full px-3 py-2 border border-black/[0.06] rounded-lg text-sm" required />
      </div>
